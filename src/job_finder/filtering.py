...
SUPPORT_POSITIVE_RULES = [
    _rule(
        "visa sponsorship",
        r"\bvisa sponsorship\b|\bsponsor(?:ing)? visa\b|\bvisa support\b",
    ),
    _rule(
        "work authorization support",
        r"\bwork authorization support\b|\bimmigration support\b",
    ),
    _rule(
        "relocation support",
        r"\brelocation (?:support|assistance|package)\b|\brelocat(?:e|ion) assistance\b",
    ),
    _rule(
        "international relocation", r"\binternational relocation\b|\bglobal mobility\b"
    ),
]
...
SUPPORT_NEGATIVE_RULES = [
    _rule(
        "no visa sponsorship",
        r"\bno (?:visa )?sponsorship\b|\bno visa sponsor(?:ship)?\b",
    ),
    _rule(
        "unable to sponsor",
        r"\bunable to sponsor\b|\bcannot sponsor\b|\bcan't sponsor\b|\bwill not sponsor\b",
    ),
    _rule(
        "no relocation",
        r"\bno relocation\b|\brelocation (?:is )?not (?:provided|available|offered)\b",
    ),
    _rule(
        "existing work authorization required",
        r"\bmust (?:already )?(?:be )?authorized to work\b|\bright to work (?:is )?required\b",
    ),
]
...
REMOTE_ELIGIBLE_RULES = [
    ...
    _rule(
        "worldwide remote",
        r"\bworldwide\b|\bglobally\b|\bglobal remote\b|\banywhere in the world\b|\bwork from anywhere\b",
    ),
...
REMOTE_RESTRICTION_RULES = [
    _rule(
        "US-only remote",
        r"\b(?:u\.?s\.?|united states|usa)[ -]?only\b|\bonly (?:in|within) (?:the )?(?:u\.?s\.?|united states|usa)\b",
    ),
    _rule("Canada-only remote", r"\bcanada[ -]?only\b|\bonly (?:in|within) canada\b"),
    _rule(
        "UK-only remote",
        r"\b(?:u\.?k\.?|united kingdom)[ -]?only\b|\bonly (?:in|within) (?:the )?(?:u\.?k\.?|united kingdom)\b",
    ),
    _rule(
        "Europe-only remote",
        r"\b(?:eu|europe|emea)[ -]?only\b|\bonly (?:in|within) (?:the )?(?:eu|europe|emea)\b",
    ),
...
class JobClassifier:
    ...
    def classify(self, job: JobPost) -> JobMatch:
        ...
        if not title_role_matches and not (
            loose_role_matching and full_text_role_matches
        ):
        ...
        if positive_support:
            ...
            return JobMatch(
                ...
                + (reasons or ["visa or relocation support found"]),
            )
        ...
        if remote_flag or remote_markers:
            ...
            return JobMatch(
                ...
                reasons=[
                    *role_reasons,
                    "remote role needs manual geography verification",
                ],
            )
...
def _labels_for_rules(rules: list[PatternRule], text: str) -> list[str]:
    return [rule.label for rule in rules if rule.pattern.search(text)]
...
def _remove_negated_support(positive: list[str], negative: list[str]) -> list[str]:
    ...
    for label in positive:
        ...
        if "visa" in label_text and any(
            term in negative_text for term in ["visa", "sponsor"]
        ):
            continue
